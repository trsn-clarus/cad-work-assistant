...
- Milestone 12 (Text Tools): no new `ui-ux-pro-max`/21st session - the CAD > Text screen reuses the existing DataGrid + right-side detail-panel shape (`HistoryPanel`, Milestone 7) for the edit table, checkbox-gated inputs (existing implicit `CheckBox` style) for batch property changes, and the app-wide Property Inspector sidebar rather than a bespoke embedded inspector - see `pages/text-tools.md`. Two real, non-obvious WPF rendering bugs were found through actual Simulation Mode screenshots (not caught by any compile step): (1) putting `Visibility="{Binding IsCreateMode}"` and `DataContext="{Binding Create}"` on the *same* element - setting `DataContext` re-resolves that element's own sibling bindings against the new context, so `Visibility` silently broke and both the edit and create panels rendered stacked on top of each other; fixed by splitting into an outer `Grid` (Visibility, parent DataContext) wrapping an inner `StackPanel` (DataContext override only). (2) The shared `ComboBox` `ControlTemplate` in `Themes/DesignTokens.xaml` renders its closed-box text via `SelectionBoxItemTemplate`, which is not auto-synthesized from `DisplayMemberPath` alone for custom-templated ComboBoxes - the color picker showed the raw `CadColorDto` type name until an explicit `ComboBox.ItemTemplate` was added (scoped to just the two new Text ComboBoxes, not the shared style, to avoid rippling into every other ComboBox in the app).
- Milestone 13 (Project Manager + File Workspace + Official User Manual PDF): no new `ui-ux-pro-max`/21st session - the new Projects screen (PROJECT group, Alt+2) is a dense list+detail split (DataGrid list `*` / `GridSplitter` / `ScrollViewer` detail pane), the same skeleton `HistoryPanel` established in Milestone 7 and `DrawingPanel` in Milestone 5, not a Card Grid (explicitly rejected by the master prompt for this screen) - see `pages/project-workspace.md`. Settings gained a new HELP section using only existing tokens (`SectionTitle`, `SecondaryButton`, `InlineMessageBorder`/`InlineMessageText` for the missing-file error) - no new component. Found the same `ComboBox` `SelectionBoxItemTemplate`-vs-`DisplayMemberPath` defect a second time, independently, in `DrawingPdfExportPanel.xaml`'s Layout selector (leaking the raw `CadPlotLayoutDto` type name) - fixed identically to Milestone 12's color picker; confirmed via `Grep` that no other `DisplayMemberPath`-only `ComboBox` remains anywhere in `Views/`. Two more real bugs surfaced only through actual Simulation Mode screenshots: a `DrawingFile` auto-registration race (a drawing already open in AutoCAD before a project existed/was switched to never got registered to it) and a resulting duplicate-row race in the Projects detail pane once the first bug was fixed (solved with a monotonic version counter + staleness guard, the same pattern `TextWorkflowViewModel.LoadDetailAsync` already used in a simpler form). The user manual PDF (not a WPF screen) reuses the existing PDF_EXPORT.md Navy/Slate/Teal palette and `WindowsKoreanFontResolver` rather than any new visual research - see `docs/ARCHITECTURE.md` §8.13.
